...
In order to classify the handwritten digit, a Convolutional Neutral Network is created and a set of instructions is given to it to classify in the input image into given categories. When the image of the handwritten digit is given as input to the model in the form of a matrix, it goes through multiple layers of mathematical operations and is compared with the set of instructions provided. At the end of the network, the image is put into one of ten categories (each representing a digit). The image is fed into the CNN in the form of a [tensor](https://www.youtube.com/watch?v=f5liqUk0ZTw).
